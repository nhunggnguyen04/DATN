...
Môi trường vận hành của một ngân hàng hiện đại thường bao gồm rất nhiều hệ thống hoạt động song song nhằm đáp ứng các nghiệp vụ khác nhau, điển hình như hệ thống Core Banking (xử lý giao dịch), CRM (quản lý quan hệ khách hàng), Card Management (quản lý thẻ) và KYC/Document Management (quản lý chứng từ định danh). Đặc thù này dẫn đến việc dữ liệu bị phân tán ở nhiều nơi và thiếu tính đồng nhất, gây ra rào cản rất lớn cho việc tổng hợp và phân tích thông tin một cách toàn diện. Từ thực trạng đó, nhu cầu cấp thiết là phải hợp nhất các nguồn dữ liệu này vào một nền tảng trung tâm. Dựa trên nguồn dữ liệu được hợp nhất, hệ thống nền tảng dữ liệu trong đồ án được định hướng giải quyết hai chủ đề phân tích (Business Intelligence) cốt lõi sau đây:

\begin{itemize}
    \item \textbf{Chủ đề 1: Phân khúc khách hàng và Marketing.} Mục tiêu nghiệp vụ của bài toán này là xác định các nhóm khách hàng tiềm năng nhằm phục vụ chiến lược up-sell các loại thẻ hoặc mời chào các gói vay. Quá trình phân tích sẽ dựa trên hành vi chi tiêu và hồ sơ tài chính của khách hàng. Kết quả đầu ra (insight) phục vụ trực tiếp cho các phòng ban như Phòng Marketing, Quản lý sản phẩm (Thẻ \& Vay), Quản lý quan hệ khách hàng (Relationship Management) và Ban lãnh đạo.
    
    \item \textbf{Chủ đề 2: Phân tích giao dịch và Quản trị rủi ro.} Bài toán này hướng tới mục tiêu cải thiện chất lượng dịch vụ và bảo vệ ngân hàng thông qua việc giảm thiểu lỗi giao dịch, thúc đẩy tỷ lệ sử dụng thẻ chip an toàn, cũng như phát hiện sớm các tín hiệu cảnh báo rủi ro vỡ nợ. Các báo cáo từ chủ đề này là công cụ đắc lực cho Phòng Vận hành (Operations), Quản trị rủi ro (Risk Management), Phân tích gian lận (Fraud Analysis), Trải nghiệm khách hàng và Giám đốc Quản trị Rủi ro (CRO).
\end{itemize}

\subsection{Dữ liệu có cấu trúc (Structured Data)}
...
\subsection{Dữ liệu phi cấu trúc (Unstructured Data)}
...
Đặc điểm lớn nhất của loại hình dữ liệu này là chúng tồn tại dưới dạng ảnh pixels, dẫn đến việc không thể thực hiện các câu truy vấn (query) trực tiếp. Để khai thác, hệ thống bắt buộc phải trải qua bước tiền xử lý bằng công nghệ nhận dạng ký tự quang học (OCR) nhằm chuyển đổi hình ảnh thành văn bản.
...
\begin{itemize}
    ...
    \item \textbf{Thiếu dấu vết kiểm toán (Audit Trail):} Các luồng nạp dữ liệu không hề ghi nhận lại lịch sử thực thi (thời gian chạy, trạng thái thành công/thất bại). Hậu quả là kỹ sư dữ liệu không có cơ sở để truy vết nguyên nhân khi xảy ra sự cố.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Đặc điểm:} Sử dụng schema cứng, tốn nhiều chi phí bản quyền phần mềm (license). Hệ thống đáp ứng tốt các yêu cầu báo cáo định kỳ cho Ngân hàng Nhà nước (NHNN).
...
\begin{itemize}
    \item \textbf{Đặc điểm:} Hệ thống Core giữ tại local (on-premise) để tuân thủ luật lưu trữ dữ liệu trong nước, trong khi luồng phân tích (workload) được đẩy lên Cloud để tận dụng khả năng mở rộng. Đã bắt đầu ứng dụng Data Lake để lưu dữ liệu phi cấu trúc và manh nha áp dụng kiến trúc Medallion.
...
\begin{itemize}
    \item \textbf{Đặc điểm:} Đây là xu hướng đích đến của ngành. Hệ thống chuyển hoàn toàn từ ETL sang ELT (nạp dữ liệu thô vào kho trước rồi mới dùng dbt để biến đổi).
...
\begin{itemize}
    ...
    \item Tích hợp nạp dữ liệu gia tăng (Incremental Loading) hàng ngày.
